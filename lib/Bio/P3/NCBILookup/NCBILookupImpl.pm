...
sub new
{
    ...
    $self->{user_agent} = P3RateLimitedUserAgent->new(3);
...
sub get_metadata_from_biosample
{
    ...
    my $xml = $self->get_xml("http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=biosample&term=$accn");
    $xml =~ s/\n//;
...
sub get_assembly_accession
{
    ...
    my $xml = $self->get_xml("http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=nuccore&term=$accn");
